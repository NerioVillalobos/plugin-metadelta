...
class FindDelta extends Command {
  ...
  async run() {
    ...
    const deletionVlocityFiles = filterCompleteVlocityDatapackDeletions(
      deletions.filter(isVlocityFile),
      fromBranch
    );
    const additionCoreFiles = additions.filter((filePath) => !isVlocityFile(filePath));
    const deletionCoreFiles = filterCompleteApexClassDeletions(
      deletions.filter((filePath) => !isVlocityFile(filePath))
    );

    const customLabelFiles = Array.from(new Set(
      [...additionCoreFiles, ...deletionCoreFiles].filter(isBundledMetadataFile)
    ));
...
export function filterCompleteVlocityDatapackDeletions(
  files,
  branch,
  hasFilesInBranch = gitPathPrefixHasFilesInBranch
) {
  const normalizedFiles = Array.from(new Set((files || []).map(normalizePath))).filter(Boolean);
