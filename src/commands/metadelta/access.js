...
class Access extends Command {
  ...
  static description =
    'Exporta aliases conectados, captura SFDX Auth URLs cifradas y restaura accesos en otro equipo (Windows/Linux/WSL) usando únicamente Node.js + Salesforce CLI.';
  ...
  runCmd(cmd, args, options = {}) {
    const result = runCommandSync(cmd, args, options);
    if (result.error || result.status !== 0) {
      throw new Error(formatCommandFailure(result, `No se pudo ejecutar: ${cmd}`));
    }
    return String(result.stdout ?? '').trim();
  }

  runJSON(cmd, args, options = {}) {
    const raw = this.runCmd(cmd, args, options);
    try {
  ...
  listConnectedOrgs() {
    ...
    return [...(result.nonScratchOrgs ?? []), ...(result.scratchOrgs ?? [])].filter(
