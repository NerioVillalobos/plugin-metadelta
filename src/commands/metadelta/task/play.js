...
class TaskPlay extends Command {
  ...
  await $1.getByRole('searchbox', {name: 'Quick Find'}).press('Enter');`
    ...
    const normalizedGenericCheckboxClicks = normalizedSetupSaveClicks
      .replace(
        /await (\w+)\.locator\('\.slds-checkbox_faux'\)\.first\(\)\.click\(\);/g,
        `await clickEinsteinToggleWithRecovery($1);`
      )
      .replace(
        /await (\w+)\.locator\('\.slds-checkbox_faux'\)\.click\(\);/g,
        `await clickEinsteinToggleWithRecovery($1);`
      );
    const normalizedNewAgentPopupFlow = normalizedGenericCheckboxClicks.replace(
      /const\s+(\w+)\s*=\s*(\w+)\.waitForEvent\('popup'\);\s*\n\s*await\s+(\w+)\.getByRole\('button',\s*\{\s*name:\s*'New Agent'\s*\}\)\.click\(\);\s*\n\s*const\s+(\w+)\s*=\s*await\s+\1;/g,
      `const $4 = await openNewAgentBuilder($2);`
    );
    const normalizedClickLogs = normalizedNewAgentPopupFlow.replace(
    ...
    const normalizedLegacyBaseUrlDeclaration = injectedImports.replace(
...
const baseUrl = (() => {
  try {
    ...
    const hasBaseUrlDeclaration =
      normalizedLegacyBaseUrlDeclaration.includes('const rawBaseUrl') ||
...
`;
    const injectedBase = normalizedLegacyBaseUrlDeclaration.replace(
      /(import\s+\{\s*test[^;]+;\n)/,
