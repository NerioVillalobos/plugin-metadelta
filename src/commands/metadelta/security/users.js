...
class MetadeltaSecurityUsers extends Command {
  ...
  async runValidationMode({flags, matrix, users, userRecords, psgRecords, groupRecords}) {
    ...
    const psgAssignments = this.queryByIds(
      flags.org,
    ...
    for (const row of validationRows) {
      ...
      if (!expected) {
        mismatches.push({
          Username: username,
          RoleName: roleName,
          MismatchReason: `Role not found in master: ${roleName}`,
          ExpectedPermissionSetGroup: '',
          CurrentPermissionSetGroup: row.PermissionSetGroup ?? '',
          ExpectedPublicGroupPuesto: '',
          CurrentPublicGroupPuesto: row.PublicGroupPuesto ?? '',
          ExpectedPublicGroupSegmento: '',
          CurrentPublicGroupSegmento: row.PublicGroupSegmento ?? '',
          ExpectedQueues: '',
          CurrentQueues: row.Queues ?? ''
        });
        continue;
      }
      ...
      const diffReasons = [];
      if (!this.setsEqual(expected.PermissionSetGroup, currentPsg)) diffReasons.push('PermissionSetGroup');
      if (!this.setsEqual(expected.PublicGroupPuesto, currentPuesto)) diffReasons.push('PublicGroupPuesto');
      if (!this.setsEqual(expected.PublicGroupSegmento, currentSegmento)) diffReasons.push('PublicGroupSegmento');
      if (!this.setsEqual(expected.Queues, currentQueues)) diffReasons.push('Queues');

      if (diffReasons.length > 0) {
        mismatches.push({
          Username: username,
          RoleName: roleName,
          MismatchReason: `Differences in: ${diffReasons.join('|')}`,
          ExpectedPermissionSetGroup: [...expected.PermissionSetGroup].sort().join(SEPARATOR),
          CurrentPermissionSetGroup: [...currentPsg].sort().join(SEPARATOR),
          ExpectedPublicGroupPuesto: [...expected.PublicGroupPuesto].sort().join(SEPARATOR),
          CurrentPublicGroupPuesto: [...currentPuesto].sort().join(SEPARATOR),
          ExpectedPublicGroupSegmento: [...expected.PublicGroupSegmento].sort().join(SEPARATOR),
          CurrentPublicGroupSegmento: [...currentSegmento].sort().join(SEPARATOR),
          ExpectedQueues: [...expected.Queues].sort().join(SEPARATOR),
          CurrentQueues: [...currentQueues].sort().join(SEPARATOR)
        });
      }
    }

    const outputPath = path.join(outputDir, COMPARE_FILE);
    writeCsv(outputPath, mismatches, [
      'Username',
      'RoleName',
      'MismatchReason',
      'ExpectedPermissionSetGroup',
      'CurrentPermissionSetGroup',
      'ExpectedPublicGroupPuesto',
      'CurrentPublicGroupPuesto',
      'ExpectedPublicGroupSegmento',
      'CurrentPublicGroupSegmento',
      'ExpectedQueues',
      'CurrentQueues'
    ]);
