...
const DEFAULT_SOLUTIONS = [
  ...
  },
  {
    ...
    description: 'Playwright encontró múltiples elementos que coinciden con un selector. Esto es común en Salesforce debido a iframes de Visualforce cacheados o elementos genéricos.',
    solution: 'Edita el script generado para asegurar que el selector sea único. Puedes añadir ".first()", ".last()", o ".filter({state: \'visible\'})" al final del localizador problemático.',
  },
  {
    pattern: '0PS[a-zA-Z0-9]+|00e[a-zA-Z0-9]+',
    description: 'El script utiliza un ID de Salesforce codificado o hardcodeado en la función "selectOption", lo cual fallará en otros entornos si el ID no existe.',
    solution: 'Edita el script generado y cambia "selectOption(\'ID\')" por "selectOption({label: \'Nombre De La Opción\'})" para que busque por texto y no por ID interno.',
  },
  {
    pattern: 'Timeout.*exceeded while waiting for event "popup"',
    description: 'Playwright esperó a que se abriera una nueva ventana o pestaña, pero nunca ocurrió.',
    solution: 'Revisa el script generado; es posible que la herramienta de grabación haya omitido el clic exacto que abre la ventana emergente (como un clic en un menú desplegable tras pulsar un botón). Agrega el click faltante en el script.',
  },
];
