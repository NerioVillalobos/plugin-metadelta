...
If you use `--metafile`, create a CommonJS file (recommended extension `.cjs`) that exports a `metadataTypes` array. For example:
...
If the file cannot be loaded or does not export the array, the command will warn and fall back to the built-in list.
...
Si usas `--metafile`, crea un archivo CommonJS (se recomienda extensión `.cjs`) que exporte un arreglo `metadataTypes`. Ejemplo:
...
Si el archivo no puede cargarse o no exporta el arreglo, el comando mostrará una advertencia y usará la lista incorporada.
