...
> **Task play hardening:** `sf metadelta task play` now includes automatic stabilizers for frontdoor/base URL separation, popup rebinds, App Launcher fallbacks, dynamic Permission Set Assignment selectors, Visualforce multiselect-combobox retries, and Action Library scroll selection + Finish enablement checks in the temporary `.metadelta.*` test file.
...
> **Robustez en task play:** `sf metadelta task play` incluye estabilizadores automáticos para separar frontdoor/base URL, reabrir popups, aplicar fallback en App Launcher, normalizar selectores dinámicos de Permission Set Assignment, reintentar combobox multiselección dentro de Visualforce y resolver selección con scroll + validación de botón Finish en Action Library dentro del archivo temporal `.metadelta.*`.
