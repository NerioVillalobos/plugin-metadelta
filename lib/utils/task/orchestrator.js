...
export async function executeCommandLive(command, args, options = {}) {
    const { cwd = process.cwd(), env = process.env, stdout = process.stdout, stderr = process.stderr, stdin = 'inherit', shell = false, } = options;
...
export function buildFrontdoorUrlFromOrgDisplay(targetOrg) {
    ...
    const displayVerbose = runSfJsonCommand(['org', 'display', '--target-org', targetOrg, '--verbose'], showSecretsOptions);
...
export function ensurePlaywrightTestDependency(baseDir = process.cwd()) {
    ...
    const result = runCommandSync('npm', ['install', '--no-fund', '--no-audit', '--prefix', cacheDir, '@playwright/test'], { stdio: 'inherit' });
    if (result.status !== 0) {
        const fallbackRuntime = resolveInstalledPlaywrightRuntime(baseDir);
        if (fallbackRuntime) {
            return fallbackRuntime;
        }
        throw new Error('No se pudo instalar @playwright/test automáticamente y tampoco se pudo resolver una instalación existente desde el proyecto actual.');
    }
    if (!fs.existsSync(cliPath)) {
        const fallbackRuntime = resolveInstalledPlaywrightRuntime(baseDir);
        if (fallbackRuntime) {
