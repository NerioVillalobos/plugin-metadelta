export class MonitorUi {
    ...
    render() {
        ...
        lines.push(row(width, labelValue('INTERVAL', `${Math.round(this.intervalMs / 60000)} min`), labelValue('RETRIEVE', formatDuration(this.retrieveDurationMs)), labelValue('NEXT', colorNextTime(formatNextTime(this.nextRefreshAt), this.nextRefreshAt)), labelValue('LAST', formatTime(this.lastRefreshAt))));
