...
async function doStartAllWindowsTerminal(context) {
    ...
    targets.forEach((target, index) => {
        ...
        args.push('new-tab', '--title', windowNameForOrg(target.org), '--startingDirectory', context.launchRoot, 'powershell.exe', '-NoExit', '-Command', buildWindowsMonitorCommand(target, context));
    });
...
function startBackgroundMonitor(target, context) {
    ...
    const executable = getCommandCandidates(context.command).find((candidate) => spawnSync(candidate, ['--version'], { stdio: 'ignore', shell: false }).status === 0) ?? context.command;
