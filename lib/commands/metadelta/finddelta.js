...
function resolveCoreComponent(filePath) {
    ...
    if (lwcMatch)
        return { type: 'LightningComponentBundle', fullName: lwcMatch[1] };
    const auraMatch = normalized.match(/\/aura\/([^/]+)\//i);
    if (auraMatch)
        return { type: 'AuraDefinitionBundle', fullName: auraMatch[1] };
    const customMetadataMatch = withoutMeta.match(/\/customMetadata\/([^/]+)\.md$/i);
    if (customMetadataMatch)
        return { type: 'CustomMetadata', fullName: customMetadataMatch[1] };
    const objectFieldMatch = withoutMeta.match(/\/objects\/([^/]+)\/fields\/([^/]+)\.field$/i);
    if (objectFieldMatch)
        return { type: 'CustomField', fullName: `${objectFieldMatch[1]}.${objectFieldMatch[2]}` };
    const objectRecordTypeMatch = withoutMeta.match(/\/objects\/([^/]+)\/recordTypes\/([^/]+)\.recordType$/i);
    if (objectRecordTypeMatch)
        return { type: 'RecordType', fullName: `${objectRecordTypeMatch[1]}.${objectRecordTypeMatch[2]}` };
    const objectValidationRuleMatch = withoutMeta.match(/\/objects\/([^/]+)\/validationRules\/([^/]+)\.validationRule$/i);
    if (objectValidationRuleMatch)
        return { type: 'ValidationRule', fullName: `${objectValidationRuleMatch[1]}.${objectValidationRuleMatch[2]}` };
    const objectListViewMatch = withoutMeta.match(/\/objects\/([^/]+)\/listViews\/([^/]+)\.listView$/i);
    if (objectListViewMatch)
        return { type: 'ListView', fullName: `${objectListViewMatch[1]}.${objectListViewMatch[2]}` };
    const objectCompactLayoutMatch = withoutMeta.match(/\/objects\/([^/]+)\/compactLayouts\/([^/]+)\.compactLayout$/i);
    if (objectCompactLayoutMatch)
        return { type: 'CompactLayout', fullName: `${objectCompactLayoutMatch[1]}.${objectCompactLayoutMatch[2]}` };
    const objectFieldSetMatch = withoutMeta.match(/\/objects\/([^/]+)\/fieldSets\/([^/]+)\.fieldSet$/i);
    if (objectFieldSetMatch)
        return { type: 'FieldSet', fullName: `${objectFieldSetMatch[1]}.${objectFieldSetMatch[2]}` };
    const objectBusinessProcessMatch = withoutMeta.match(/\/objects\/([^/]+)\/businessProcesses\/([^/]+)\.businessProcess$/i);
    if (objectBusinessProcessMatch)
        return { type: 'BusinessProcess', fullName: `${objectBusinessProcessMatch[1]}.${objectBusinessProcessMatch[2]}` };
    const objectWebLinkMatch = withoutMeta.match(/\/objects\/([^/]+)\/webLinks\/([^/]+)\.webLink$/i);
    if (objectWebLinkMatch)
        return { type: 'WebLink', fullName: `${objectWebLinkMatch[1]}.${objectWebLinkMatch[2]}` };
    const objectPathAssistantMatch = withoutMeta.match(/\/objects\/([^/]+)\/compactLayouts\/([^/]+)\.compactLayout$/i);
    if (objectPathAssistantMatch)
        return { type: 'CompactLayout', fullName: `${objectPathAssistantMatch[1]}.${objectPathAssistantMatch[2]}` };
    const customObjectMatch = withoutMeta.match(/\/objects\/([^/]+)\/([^/]+)\.object$/i);
    if (customObjectMatch)
        return { type: 'CustomObject', fullName: customObjectMatch[1] };
    return null;
}
