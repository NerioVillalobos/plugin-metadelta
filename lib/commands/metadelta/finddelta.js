...
// These metadata files contain multiple package members in one physical XML file.
// Keep the mapping explicit: other XML collections (for example Workflow rules)
// are children of one package member and must not be split into separate members.
const BUNDLED_METADATA = [
    {
        file: /(?:^|\/)CustomLabels\.labels-meta\.xml$/i,
        root: 'CustomLabels',
        collection: 'labels',
        memberType: 'CustomLabel',
        memberKey: 'fullName'
    }
];
class FindDelta extends Command {
    ...
    async run() {
        ...
        const deletionVlocityFiles = filterCompleteVlocityDatapackDeletions(deletions.filter(isVlocityFile), fromBranch);
        const additionCoreFiles = additions.filter((filePath) => !isVlocityFile(filePath));
        const deletionCoreFiles = filterCompleteApexClassDeletions(deletions.filter((filePath) => !isVlocityFile(filePath)));
        const customLabelFiles = Array.from(new Set([...additionCoreFiles, ...deletionCoreFiles].filter(isBundledMetadataFile)));
        const standardAdditionCoreFiles = additionCoreFiles.filter((filePath) => !isBundledMetadataFile(filePath));
        const standardDeletionCoreFiles = deletionCoreFiles.filter((filePath) => !isBundledMetadataFile(filePath));
        const bundledMetadataDelta = buildBundledMetadataDeltaComponents(fromBranch, toBranch, customLabelFiles);
        const activeCore = dedupeComponents([
            ...buildCoreComponentsFromBranchFiles(fromBranch, standardAdditionCoreFiles, this),
            ...bundledMetadataDelta.active
        ]);
        const destructiveCore = dedupeComponents([
            ...buildCoreComponentsFromBranchFiles(toBranch, standardDeletionCoreFiles, this),
            ...bundledMetadataDelta.destructive
        ]);
...
export function isBundledMetadataFile(filePath) {
    return Boolean(getBundledMetadataDefinition(filePath));
}
export function buildBundledMetadataDeltaComponents(fromBranch, toBranch, files, readFile = readBranchFile) {
    const active = [];
    const destructive = [];
    for (const filePath of Array.from(new Set((files || []).map(normalizePath))).filter(isBundledMetadataFile)) {
        const definition = getBundledMetadataDefinition(filePath);
        const sourceLabels = parseBundledMetadata(readFile(fromBranch, filePath), definition);
        const targetLabels = parseBundledMetadata(readFile(toBranch, filePath), definition);
        const targetByName = new Map(targetLabels.map((label) => [label.fullName, label.signature]));
        const sourceByName = new Map(sourceLabels.map((label) => [label.fullName, label.signature]));
        for (const label of sourceLabels) {
            if (!targetByName.has(label.fullName) || targetByName.get(label.fullName) !== label.signature) {
                active.push({ type: definition.memberType, fullName: label.fullName });
            }
        }
        for (const label of targetLabels) {
            if (!sourceByName.has(label.fullName)) {
                destructive.push({ type: definition.memberType, fullName: label.fullName });
            }
        }
    }
    return { active: dedupeComponents(active), destructive: dedupeComponents(destructive) };
}
function getBundledMetadataDefinition(filePath) {
    const normalized = normalizePath(filePath);
    return BUNDLED_METADATA.find((definition) => definition.file.test(normalized));
}
...
function parseBundledMetadata(fileContent, definition) {
    if (!fileContent)
        return [];
    try {
        const parser = new XMLParser({ ignoreAttributes: false, processEntities: true });
        const parsed = parser.parse(fileContent);
        const members = parsed?.[definition.root]?.[definition.collection];
        const memberArray = members == null ? [] : (Array.isArray(members) ? members : [members]);
        return memberArray
            .map((label) => ({
            fullName: String(label?.[definition.memberKey] || '').trim(),
            signature: JSON.stringify(canonicalizeXmlValue(label))
        }))
...
function canonicalizeXmlValue(value) {
    if (Array.isArray(value)) {
        return value.map(canonicalizeXmlValue);
    }
    if (value && typeof value === 'object') {
        return Object.fromEntries(Object.keys(value)
            .sort()
            .map((key) => [key, canonicalizeXmlValue(value[key])]));
    }
    return value;
}
...
export function filterCompleteApexClassDeletions(files) {
    const normalizedFiles = Array.from(new Set((files || []).map(normalizePath))).filter(Boolean);
    const deletedSet = new Set(normalizedFiles.map((filePath) => filePath.toLowerCase()));
    const filtered = [];
    for (const filePath of normalizedFiles) {
        const normalized = normalizePath(filePath);
        const lower = normalized.toLowerCase();
        if (lower.endsWith('.cls-meta.xml')) {
            const classPath = normalized.replace(/\.cls-meta\.xml$/i, '.cls');
            if (deletedSet.has(classPath.toLowerCase())) {
                filtered.push(normalized);
            }
            continue;
        }
        if (lower.endsWith('.cls')) {
            const metaPath = `${normalized}-meta.xml`;
            if (deletedSet.has(metaPath.toLowerCase())) {
                filtered.push(normalized);
            }
            continue;
        }
...
export function filterCompleteVlocityDatapackDeletions(files, branch, hasFilesInBranch = gitPathPrefixHasFilesInBranch) {
    const normalizedFiles = Array.from(new Set((files || []).map(normalizePath))).filter(Boolean);
    const datapackRoots = new Map();
    for (const filePath of normalizedFiles) {
        const datapackRoot = resolveVlocityDatapackRoot(filePath);
        if (!datapackRoot) {
            continue;
        }
        if (!datapackRoots.has(datapackRoot.toLowerCase())) {
            datapackRoots.set(datapackRoot.toLowerCase(), datapackRoot);
        }
    }
    const fullyDeletedRoots = new Set();
    for (const [rootKey, datapackRoot] of datapackRoots.entries()) {
        if (!hasFilesInBranch(branch, datapackRoot)) {
            fullyDeletedRoots.add(rootKey);
        }
    }
    return normalizedFiles.filter((filePath) => {
        const datapackRoot = resolveVlocityDatapackRoot(filePath);
        return datapackRoot && fullyDeletedRoots.has(datapackRoot.toLowerCase());
    });
}
function gitPathPrefixHasFilesInBranch(branch, pathPrefix) {
    const output = execFileSync('git', ['ls-tree', '-r', '--name-only', branch, '--', pathPrefix], {
        encoding: 'utf8'
    });
    return output.split('\n').some((line) => line.trim());
}
function resolveVlocityDatapackRoot(filePath) {
    const normalized = normalizePath(filePath);
    const segments = normalized.split('/').filter(Boolean);
    const vlocityIndex = segments.findIndex((segment) => segment.toLowerCase() === 'vlocity');
    if (vlocityIndex < 0 || segments.length < vlocityIndex + 3) {
        return null;
    }
    return segments.slice(0, vlocityIndex + 3).join('/');
}
...
function mergeIntoXmlManifest(targetPath, components) {
    ...
    for (const component of components) {
        ...
        if (!members.includes(component.fullName)) {
            members.push(component.fullName);
        }
    }
    for (const [typeName, membersToAdd] of grouped.entries()) {
        content = mergeMembersIntoTypeBlock(content, typeName, membersToAdd, eol);
    }
    fs.writeFileSync(resolvedPath, content, 'utf8');
    return resolvedPath;
}
function mergeMembersIntoTypeBlock(xmlContent, typeName, membersToAdd, eol) {
    const typeBlock = findTypeBlockByName(xmlContent, typeName);
    if (!typeBlock) {
        return appendNewTypeBlock(xmlContent, typeName, membersToAdd, eol);
    }
    const existingMembers = new Set();
    const memberRegex = /<members>\s*([\s\S]*?)\s*<\/members>/g;
    let memberMatch;
    while ((memberMatch = memberRegex.exec(typeBlock.block)) !== null) {
        const memberName = String(memberMatch[1] || '').trim();
        if (memberName) {
            existingMembers.add(memberName);
        }
    }
    const missingMembers = membersToAdd.filter((member) => !existingMembers.has(member));
    if (missingMembers.length === 0) {
        return xmlContent;
    }
    const memberIndentMatch = typeBlock.block.match(/(^[	 ]*)<members>/m);
    const nameIndentMatch = typeBlock.block.match(/(^[	 ]*)<name>/m);
    const memberIndent = memberIndentMatch?.[1] ?? `${nameIndentMatch?.[1] ?? '    '}    `;
    const membersToInsert = missingMembers
        .map((member) => `${memberIndent}<members>${escapeXml(member)}</members>`)
        .join(eol);
    const updatedBlock = typeBlock.block.replace(/(^[	 ]*<name>)/m, `${membersToInsert}${eol}$1`);
    return `${xmlContent.slice(0, typeBlock.start)}${updatedBlock}${xmlContent.slice(typeBlock.end)}`;
}
function findTypeBlockByName(xmlContent, typeName) {
    const blockRegex = /<types>[\s\S]*?<name>\s*([^<]+)\s*<\/name>[\s\S]*?<\/types>/g;
    let match;
    while ((match = blockRegex.exec(xmlContent)) !== null) {
        const candidateName = String(match[1] || '').trim();
        if (candidateName === typeName) {
            return { start: match.index, end: blockRegex.lastIndex, block: match[0] };
        }
    }
    return null;
}
function appendNewTypeBlock(xmlContent, typeName, membersToAdd, eol) {
    if (!membersToAdd || membersToAdd.length === 0) {
        return xmlContent;
    }
    const typeIndentMatch = xmlContent.match(/(^[	 ]*)<types>/m);
    const typeIndent = typeIndentMatch?.[1] ?? '    ';
    const memberIndent = `${typeIndent}    `;
    const uniqueMembers = [];
    ...
    const blockLines = [
        `${typeIndent}<types>`,
        ...uniqueMembers.map((member) => `${memberIndent}<members>${escapeXml(member)}</members>`),
        `${memberIndent}<name>${escapeXml(typeName)}</name>`,
        `${typeIndent}</types>`
    ];
    const newBlock = blockLines.join(eol);
    const versionMatch = xmlContent.match(/^[	 ]*<version>[\s\S]*?<\/version>\s*$/m);
    if (versionMatch && versionMatch.index !== undefined) {
        const insertAt = versionMatch.index;
        return `${xmlContent.slice(0, insertAt)}${newBlock}${eol}${xmlContent.slice(insertAt)}`;
    }
    const closeTag = '</Package>';
    const closeTagIndex = xmlContent.lastIndexOf(closeTag);
    if (closeTagIndex < 0) {
        return `${xmlContent}${eol}${newBlock}${eol}`;
    }
    return `${xmlContent.slice(0, closeTagIndex)}${newBlock}${eol}${xmlContent.slice(closeTagIndex)}`;
}
