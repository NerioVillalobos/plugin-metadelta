...
class GitAnalyze extends Command {
  ...
  static flags = {
    ...
    'ai-provider': Flags.string({
      summary: 'Proveedor de IA (openai | gemini)',
      default: 'openai'
    }),
    'ai-model': Flags.string({
      summary: 'Modelo de IA a usar (si se omite: gpt-4o-mini para OpenAI, gemini-2.0-flash para Gemini)'
    })
  };
  ...
  async run() {
    ...
    if (flags.ai) {
      this.log(`ℹ️ IA solicitada: ${aiProvider.toUpperCase()} (${aiModel})`);
    }
