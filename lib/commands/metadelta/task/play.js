...
class TaskPlay extends Command {
  ...
  await $1.getByRole('searchbox', {name: 'Quick Find'}).press('Enter');`);
        ...
        const normalizedBaseUrlExpressions = normalizedBaseUrls
            ...
            .replace(/"baseUrl(\/[^"]*)"/g, "baseUrl + '$1'");
        const setupPopupPattern = new RegExp("const page1Promise = page\.waitForEvent\('popup'\);\s*\n\s*await page\.getByRole\('menuitem', \{ name: 'Setup Opens in a new tab Setup for current app' \}\)\.click\(\);\s*\n\s*const page1 = await page1Promise;\s*\n\s*await page1\.goto\([^;]+\);", 'g');
        const normalizedSetupPopup = normalizedBaseUrlExpressions.replace(setupPopupPattern, `const page1 = await openSetupPage(page);`);
        const normalizedCheckboxes = normalizedSetupPopup.replace(/await (\w+)\.locator\('iframe\[name\^="vfFrameId_"\]'\)\.contentFrame\(\)\.getByRole\('checkbox', \{ name: '([^']+)' \}\)\.(check|uncheck)\(\);/g, `{
