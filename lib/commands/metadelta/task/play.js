...
class TaskPlay extends Command {
  ...
  testMatch: [${JSON.stringify(testBase)}],
    ...
    normalizeVisualforceComboboxSelections(source) {
        const sequenceRegex = /await (\w+)\.locator\('iframe\[name\^="vfFrameId_"\]'\)\.contentFrame\(\)\.getByRole\('combobox', \{ name: '([^']+)' \}\)\.click\(\);\s*((?:await \1\.locator\('iframe\[name\^="vfFrameId_"\]'\)\.contentFrame\(\)\.getByText\('([^']+)'\)\.click\(\);\s*){1,8})/g;
        return source.replace(sequenceRegex, (match, pageVar, comboboxLabel, optionBlock) => {
            const optionLabels = [...optionBlock.matchAll(/getByText\('([^']+)'\)\.click\(\);/g)].map((entry) => entry[1]);
  ...
  await $1.getByRole('searchbox', {name: 'Quick Find'}).press('Enter');`);
    ...
    if (vf) {
      ...
      await editAssignmentsButton.click({timeout: 15000});
        ...
        const normalizedActionLibraryCheckboxes = normalizedComboboxSelections.replace(/await (\w+)\.locator\('#check-button-label-[^']+ > \.slds-checkbox_faux'\)\.click\(\);/g, `await selectActionLibraryCheckboxWithScroll($1);`);
