...
class TaskPlay extends Command {
  ...
  await page
    ...
    .click({force: true});
  await ensureStartTriggered(page);`);
        const normalizedMaintenanceWaits = normalizedExactStartClicks.replace(/await page\.locator\('iframe\[name\^="vfFrameId_"\]'\)\.contentFrame\(\)\.locator\('([^']*job-start[^']*)'\)\.click\(\);/g, `await page
    .locator('iframe[name^="vfFrameId_"]')
    .contentFrame()
    .locator('$1')
    .click();
