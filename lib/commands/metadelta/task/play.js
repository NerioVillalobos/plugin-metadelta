...
class TaskPlay extends Command {
  ...
  await page
    ...
    .click({force: true});
  await ensureStartTriggered(page);`);
        const normalizedDeliverabilityClick = normalizedExactStartClicks.replace(/await (\w+)\.getByRole\('link', \{ name: 'Deliverability', exact: true \}\)\.click\(\);/g, `{
    ...
    if (await deliverabilityLink.count()) {
      ...
      await $1.getByText('Deliverability', {exact: true}).first().click({timeout: 15000});
        ...
        const injectedImports = normalizedDeliverabilityClick.replace(/(import\s+\{\s*test[^;]+;)/, `$1\nimport {runTaskOrchestrator} from './metadelta-task-orchestrator-routes.js';`);
