  ...
  await $1.getByRole('searchbox', {name: 'Quick Find'}).press('Enter');`);
    ...
    if (await saveButton.count()) {
      ...
      console.log('⚠️ Se omite Save porque no existe botón Save visible en el contexto actual.');
        ...
        const normalizedVisualforceComboboxSelections = normalizedSetupSaveClicks.replace(/await (\w+)\.locator\('iframe\[name\^="vfFrameId_"\]'\)\.contentFrame\(\)\.getByRole\('combobox', \{ name: '([^']+)' \}\)\.click\(\);\s*((?:\s*await \1\.locator\('iframe\[name\^="vfFrameId_"\]'\)\.contentFrame\(\)\.getByText\('([^']+)'\)\.click\(\);\s*)+)/g, (match, pageRef, comboboxName, optionBlock) => {
        ...
        const normalizedClickLogs = normalizedVisualforceComboboxSelections.replace(/await (\w+)\.getByRole\('searchbox', \{ name: 'Quick Find' \}\)\.press\('Enter'\);/g, `console.log('➡️ Enter: Quick Find');\n  await $1.getByRole('searchbox', {name: 'Quick Find'}).press('Enter');`);
