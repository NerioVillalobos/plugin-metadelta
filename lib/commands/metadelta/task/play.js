  ...
  await $1.getByRole('searchbox', {name: 'Quick Find'}).press('Enter');`);
    ...
    if (checkbox) {
      ...
      await markSetupSectionReady();
        ...
        const normalizedSetupSaveClicks = normalizedCheckboxes.replace(/await (\w+)\.locator\('iframe\[name\^="vfFrameId_"\]'\)\.contentFrame\(\)\.getByRole\('button', \{ name: 'Save' \}\)\.click\(\);/g, `if (isSetupSectionReady()) {
    await $1
      .locator('iframe[name^="vfFrameId_"]')
      .contentFrame()
