  ...
  await waitForMaintenanceJob();`);
    ...
    if (await uiLinks.nth(1).count()) {
      ...
      await uiLinks.first().click({timeout: 15000, force: true});
        ...
        const normalizedAgentforceAgentsClick = normalizedUserInterfaceClick.replace(/await (\w+)\.getByRole\('link', \{ name: 'Agentforce Agents' \}\)\.click\(\);/g, `await clickSetupLinkWithRecovery($1, 'Agentforce Agents', {
