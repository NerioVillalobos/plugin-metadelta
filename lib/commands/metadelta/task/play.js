  ...
  await $1.getByRole('searchbox', {name: 'Quick Find'}).press('Enter');`);
    ...
    if ((await agentforceLink.count()) > 0) {
      ...
      await $1.getByText('Agentforce Agents', {exact: true}).first().click({timeout: 15000, force: true});
        ...
        const normalizedIframeHtmlClicks = normalizedAgentforceLink.replace(/await (\w+)\.locator\('iframe\[name\^="vfFrameId_"\]'\)\.contentFrame\(\)\.locator\('html'\)\.click\(\);/g, `// omit iframe html click in patched tests to avoid timeouts in other orgs`);
