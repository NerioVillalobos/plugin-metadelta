...
class TaskPlay extends Command {
  ...
  testMatch: [${JSON.stringify(testBase)}],
    ...
    async maybeCreateAiEnhancedTestFile({ aiEnabled, aiProvider, aiKey, aiModel, originalTestFile, patchedTestFile }) {
        ...
        if (!resolvedKey) {
            this.warn('AI habilitada sin credenciales. Usa --ai-key (o GEMINI_API_KEY / METADELTA_AI_KEY). Se usará el archivo determinista.');
            return {
                enabled: true,
                provider,
                model: null,
        ...
        try {
            ...
            if (!syntaxValidation.valid) {
                this.warn(`La salida IA fue inválida para TypeScript/Playwright (${syntaxValidation.reason}). Se usará el archivo determinista.`);
                return {
                    enabled: true,
                    provider,
                    model,
    ...
    isValidAiPatchedTestContent(content) {
        ...
        const preservesMetadeltaCore = normalized.includes('METADELTA_HELPERS_BEGIN') &&
    ...
    validateAiTypescriptSyntax(content, patchedTestFile) {
        ...
        const sourceFile = ts.createSourceFile(this.createAiEnhancedTestFilePath(patchedTestFile), content, ts.ScriptTarget.Latest, true, ts.ScriptKind.TS);
    ...
    applyAiHardeningPlan(source, changes) {
        ...
        if (changeTypes.has('setup_button_disambiguation')) {
            updated = updated.replace(/await\s+page\.getByRole\('button',\s*\{\s*name:\s*'Setup'\s*\}\)\.click\(\);/g, `{
    ...
    if ((await globalSetupButton.count()) > 0) {
      ...
      await setupButtonExact.click({timeout: 15000});
        ...
        if (changeTypes.has('quick_find_ready_guard')) {
            updated = updated
                .replace(/await (\w+)\.getByRole\('searchbox', \{ name: 'Quick Find' \}\)\.click\(\);/g, `await $1.getByRole('searchbox', {name: 'Quick Find'}).waitFor({state: 'visible', timeout: 15000});
  await $1.getByRole('searchbox', {name: 'Quick Find'}).click({timeout: 15000});`)
                .replace(/await (\w+)\.getByRole\('searchbox', \{ name: 'Quick Find' \}\)\.fill\('([^']+)'\);/g, `await $1.getByRole('searchbox', {name: 'Quick Find'}).waitFor({state: 'visible', timeout: 15000});
  await $1.getByRole('searchbox', {name: 'Quick Find'}).fill('$2');`);
        }
        if (changeTypes.has('iframe_readiness_guard')) {
            updated = updated.replace(/const vf = await (\w+)\.locator\('iframe\[name\^="vfFrameId_"\]'\)\.first\(\)\.contentFrame\(\);/g, `const vfLocator = $1.locator('iframe[name^="vfFrameId_"]').first();
  await vfLocator.waitFor({timeout: 15000});
  const vf = await vfLocator.contentFrame();`);
        }
        if (changeTypes.has('click_ready_guard')) {
            updated = updated.replace(/await (\w+)\.getByRole\('button', \{ name: 'OK' \}\)\.click\(\);/g, `await $1.getByRole('button', {name: 'OK'}).waitFor({state: 'visible', timeout: 15000});
  await $1.getByRole('button', {name: 'OK'}).click({timeout: 15000});`);
    ...
    async resolveGeminiModel({ apiKey, preferredModel }) {
        ...
        try {
            ...
            const compatible = models.filter((entry) => Array.isArray(entry?.supportedGenerationMethods) && entry.supportedGenerationMethods.includes('generateContent'));
    ...
    normalizeSetupRefreshCloseReopenSequence(source) {
        return source.replace(/await\s+(page\d+)\.keyboard\.press\('Control\+R'\);\n\s*await\s+\1\.close\(\);\n\s*const\s+(page\d+Reopened)\s*=\s*await\s+openSetupPopup\(page\);/g, `await forceFullPageRefresh($1);\n  await $1.close();\n  const $2 = await openSetupPopup(page);`);
    }
...
`;
  ...
  await $1.getByRole('searchbox', {name: 'Quick Find'}).press('Enter');`);
        const normalizedAgentforceLink = normalizedQuickFind.replace(/await (\w+)\.getByRole\('link', \{ name: 'Agentforce Agents' \}\)\.click\(\);/g, `await clickAgentforceAgentsLink($1);`);
    ...
    if (vf) {
      ...
      await editAssignmentsButton.click({timeout: 15000});
        ...
        const normalizedGenericCheckboxFauxClicks = this.shouldPreserveLegacyCriticalFlowSelectors(normalizedActionLibraryCheckboxes)
            ...
            : normalizedActionLibraryCheckboxes.replace(/await (\w+)\.locator\('\.slds-checkbox_faux'\)\.click\(\);/g, `await clickCheckboxFaux($1);`);
        const normalizedCheckboxes = normalizedGenericCheckboxFauxClicks.replace(/await (\w+)\.locator\('iframe\[name\^="vfFrameId_"\]'\)\.contentFrame\(\)\.getByRole\('checkbox', \{ name: '([^']+)' \}\)\.(check|uncheck)\(\);/g, `{
    ...
    if (await saveButton.count()) {
      ...
      console.log('⚠️ Se omite Save porque no existe botón Save visible en el contexto actual.');
        ...
        const normalizedSetupSaveInputClicks = normalizedSetupSaveClicks.replace(/await (\w+)\.locator\('iframe\[name\^="vfFrameId_"\]'\)\.contentFrame\(\)\.getByRole\('row', \{ name: '([^']+)', exact: true \}\)\.locator\('input\[name="save"\]'\)\.click\(\);/g, `{
    const saveInput = $1
      .locator('iframe[name^="vfFrameId_"]')
      .contentFrame()
    ...
    if ((await saveInput.count()) > 0) {
      ...
      console.log('⚠️ Se omite Save input porque no se encontró input[name=\"save\"] visible.');
        ...
        const normalizedIdempotentCheckboxes = normalizedSetupSaveInputClicks.replace(/await ([^;\n]+?getByRole\('checkbox', \{ name: '([^']+)' \}[^;\n]*)\.(check|uncheck)\(\);/g, (match, locatorExpression, checkboxName, operation) => {
        ...
        const normalizedIdempotentSwitches = normalizedIdempotentCheckboxes.replace(/await ([^;\n]+?getByRole\('switch', \{ name: '([^']+)' \}[^;\n]*)\.click\(\);/g, (match, locatorExpression, switchName) => `await clickToggleIfNeeded(${locatorExpression}, true, 'switch:${switchName}');`);
        const normalizedIdempotentFills = normalizedIdempotentSwitches.replace(/await ([^;\n]+?)\.fill\('([^']*)'\);/g, (match, locatorExpression, desiredValue) => `await fillIfNeeded(${locatorExpression}, '${desiredValue.replace(/'/g, "\\'")}', 'fill:${desiredValue.slice(0, 32)}');`);
        const normalizedOffThenToggle = normalizedIdempotentFills.replace(/await expect\(([\w$]+\.locator\('[^']+'\))\.getByText\('Off'\)\)\.toBeVisible\(\);\s*\n\s*await ([^;\n]+?\.locator\('\.slds-checkbox_faux'\)\.first\(\))\.click\(\);/g, `await ensureToggleStateFromDescription($1, $2, 'Off', 'On', 'toggle-from-off-expectation');`);
        const normalizedClickLogs = normalizedOffThenToggle.replace(/await (\w+)\.getByRole\('searchbox', \{ name: 'Quick Find' \}\)\.press\('Enter'\);/g, `console.log('➡️ Enter: Quick Find');\n  await $1.getByRole('searchbox', {name: 'Quick Find'}).press('Enter');`);
...
async function openSetupPopup(page, options = {}) {
  ...
  const isLikelySetupPage = async (candidatePage) => {
    try {
      const currentUrl = candidatePage.url();
      if (/\\/lightning\\/setup\\//i.test(currentUrl) || /salesforce-setup\\.com/i.test(currentUrl)) {
        return true;
      }
    } catch (error) {
      // noop
    ...
    try {
      const quickFind = candidatePage.getByRole('searchbox', {name: 'Quick Find'}).first();
      return (await quickFind.count()) > 0;
    } catch (error) {
      return false;
    }
  };

  if (await isLikelySetupPage(page)) {
    return page;
  }

  const contextPages = page.context().pages();
  for (const candidate of [...contextPages].reverse()) {
    if (candidate === page) {
      continue;
    }
    if (await isLikelySetupPage(candidate)) {
...
`;
    ...
    validatePatchedTestStructure(contents, patchedPath) {
        ...
        if (helperBeginCount !== 1 || helperEndCount !== 1) {
            failures.push(`Se esperaba exactamente 1 bloque de helpers marcado (BEGIN/END) y se encontraron BEGIN=${helperBeginCount}, END=${helperEndCount}.`);
        }
        if (failures.length > 0) {
            throw new Error(`El archivo temporal parcheado no pasó validación estructural (${patchedPath}).\n- ${failures.join('\n- ')}`);
        }
    }
    detectLegacyOrPartialHelperIssue(contents) {
        ...
        const detectedSignals = helperSignals.filter((name) => new RegExp(`\\basync\\s+function\\s+${name}\\s*\\(`).test(contents));
