...
import { TaskOrchestrator, ensureTestsDirectory, ensurePlaywrightReady, buildFrontdoorUrlFromOrgDisplay, ensurePlaywrightTestDependency, resolveTestFilePath, } from '../../../utils/task/orchestrator.js';
  ...
  await waitForMaintenanceJob();`);
    ...
    if (await uiLinks.nth(1).count()) {
      ...
      await uiLinks.first().click({timeout: 15000, force: true});
        ...
        const normalizedEinsteinSetupClick = normalizedUserInterfaceClick.replace(/await (\w+)\.getByRole\('link', \{ name: 'Einstein Setup' \}\)\.click\(\);/g, `await clickSetupLinkWithRecovery($1, 'Einstein Setup', {
    setupLandingPath: '/lightning/setup/EinsteinGPTSetup/home',
  });`);
        const normalizedAgentforceAgentsClick = normalizedEinsteinSetupClick.replace(/await (\w+)\.getByRole\('link', \{ name: 'Agentforce Agents' \}\)\.click\(\);/g, `await clickSetupLinkWithRecovery($1, 'Agentforce Agents', {
    setupLandingPath: '/lightning/setup/EinsteinGPTSetup/home',
  });`);
        const normalizedQuickFindClick = this.shouldEnableQuickFindFallback()
            ? normalizedAgentforceAgentsClick.replace(/await (\w+)\.getByRole\('searchbox', \{ name: 'Quick Find' \}\)\.click\(\);/g, `{
    ...
    await quickFindSearchbox.click({timeout: 20000});
        ...
        const normalizedQuickFind = normalizedQuickFindClick.replace(/await (\w+)\.getByRole\('searchbox', \{ name: 'Quick Find' \}\)\.fill\('([^']+)'\);/g, `await $1.getByRole('searchbox', {name: 'Quick Find'}).fill('$2');
  await $1.getByRole('searchbox', {name: 'Quick Find'}).press('Enter');`);
        const normalizedIframeHtmlClicks = normalizedQuickFind.replace(/await (\w+)\.locator\('iframe\[name\^="vfFrameId_"\]'\)\.contentFrame\(\)\.locator\('html'\)\.click\(\);/g, `// omit iframe html click in patched tests to avoid timeouts in other orgs`);
        ...
        const normalizedBaseUrlExpressions = normalizedBaseUrls
            ...
            .replace(/"baseUrl(\/[^"]*)"/g, "baseUrl + '$1'");
        const normalizedCheckboxes = normalizedBaseUrlExpressions.replace(/await (\w+)\.locator\('iframe\[name\^="vfFrameId_"\]'\)\.contentFrame\(\)\.getByRole\('checkbox', \{ name: '([^']+)' \}\)\.(check|uncheck)\(\);/g, `{
    ...
    if (checkbox) {
      ...
      await checkbox.$3({timeout: 15000});
        ...
        const normalizedSetupSaveClicks = normalizedCheckboxes.replace(/await (\w+)\.locator\('iframe\[name\^="vfFrameId_"\]'\)\.contentFrame\(\)\.getByRole\('button', \{ name: 'Save' \}\)\.click\(\);/g, `{
    const saveButton = $1
      .locator('iframe[name^="vfFrameId_"]')
      .contentFrame()
    ...
    if (await saveButton.count()) {
      ...
      console.log('⚠️ Se omite Save porque no existe botón Save visible en el contexto actual.');
        ...
        const normalizedGenericCheckboxClicks = normalizedSetupSaveClicks
            .replace(/await (\w+)\.locator\('\.slds-checkbox_faux'\)\.first\(\)\.click\(\);/g, `await clickEinsteinToggleWithRecovery($1);`)
            .replace(/await (\w+)\.locator\('\.slds-checkbox_faux'\)\.click\(\);/g, `await clickEinsteinToggleWithRecovery($1);`);
        const normalizedNewAgentPopupFlow = normalizedGenericCheckboxClicks.replace(/const\s+(\w+)\s*=\s*(\w+)\.waitForEvent\('popup'\);\s*\n\s*await\s+(\w+)\.getByRole\('button',\s*\{\s*name:\s*'New Agent'\s*\}\)\.click\(\);\s*\n\s*const\s+(\w+)\s*=\s*await\s+\1;/g, `const $4 = await openNewAgentBuilder($2);`);
        const normalizedClickLogs = normalizedNewAgentPopupFlow.replace(/await (\w+)\.getByRole\('searchbox', \{ name: 'Quick Find' \}\)\.press\('Enter'\);/g, `console.log('➡️ Enter: Quick Find');\n  await $1.getByRole('searchbox', {name: 'Quick Find'}).press('Enter');`);
        ...
        const normalizedLegacyBaseUrlDeclaration = injectedImports.replace(/const\s+baseUrl\s*=\s*process\.env\.METADELTA_BASE_URL\s*\?\?\s*baseUrl\s*;/g, `const rawBaseUrl = process.env.METADELTA_BASE_URL ?? '';
const baseUrl = (() => {
  try {
    ...
    return rawBaseUrl;
        ...
        const hasBaseUrlDeclaration = normalizedLegacyBaseUrlDeclaration.includes('const rawBaseUrl') ||
            normalizedLegacyBaseUrlDeclaration.includes('const baseUrl');
        const baseUrlDeclaration = hasBaseUrlDeclaration
            ? ''
...
`;
        const injectedBase = normalizedLegacyBaseUrlDeclaration.replace(/(import\s+\{\s*test[^;]+;\n)/, `$1${baseUrlDeclaration}`);
...
async function clickSetupLinkWithRecovery(page, linkName, options = {}) {
  ...
  const urlLooksReady = () => page.url().includes(setupLandingPath) || page.url().includes('/lightning/setup/');
...
async function clickEinsteinToggleWithRecovery(page) {
  await clickSetupLinkWithRecovery(page, 'Einstein Setup', {
    setupLandingPath: '/lightning/setup/EinsteinGPTSetup/home',
  });

  const candidates = [
    page.locator('label:has-text("Turn on Einstein") .slds-checkbox_faux'),
    page.locator('.slds-checkbox_faux'),
  ];
  ...
  for (const candidate of candidates) {
    ...
    for (let index = 0; index < count; index += 1) {
      ...
      if (await current.isVisible()) {
        ...
        await current.click({timeout: 15000, force: true});
        return;
      }
    }
  }

  throw new Error('No se encontró un toggle visible (.slds-checkbox_faux) en Einstein Setup.');
}
