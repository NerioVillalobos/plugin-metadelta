  ...
  await $1.getByRole('searchbox', {name: 'Quick Find'}).press('Enter');`);
    ...
    if (vf) {
      ...
      await editAssignmentsButton.click({timeout: 15000});
        ...
        const normalizedGenericCheckboxFauxClicks = normalizedActionLibraryCheckboxes.replace(/await (\w+)\.locator\('\.slds-checkbox_faux'\)\.click\(\);/g, `await clickCheckboxFaux($1);`);
...
async function clickAgentforceAgentsLink(page, options = {}) {
  const {reloadWaitMs = 3000} = options;
  const quickFind = page.getByRole('searchbox', {name: 'Quick Find'}).first();
  const agentforceLink = page.getByRole('link', {name: 'Agentforce Agents'}).first();
  const agentforceTextExact = page.getByText('Agentforce Agents', {exact: true}).first();
  const agentforceTextLoose = page.getByText(/Agentforce Agents/i).first();
  ...
  if ((await agentforceTextExact.count()) > 0) {
    await agentforceTextExact.click({timeout: 20000, force: true});
    return;
  }

  if ((await agentforceTextLoose.count()) > 0) {
    await agentforceTextLoose.click({timeout: 20000, force: true});
    return;
  }

  throw new Error('No se pudo ubicar Agentforce Agents después de Quick Find y refresh de compatibilidad.');
}
