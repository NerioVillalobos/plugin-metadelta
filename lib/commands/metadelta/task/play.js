...
class TaskPlay extends Command {
  ...
  testMatch: [${JSON.stringify(testBase)}],
    ...
    createPatchedTestFile(testFile, vlocityJobTime) {
        ...
        const normalizedButtons = this.shouldNormalizeGenericButtonSelectors()
            ? normalizedDynamicVisualforceIds.replace(/contentFrame\(\)\.locator\('button:nth-child\(2\)'\)/g, "contentFrame().getByRole('button', { name: /Start/i })")
  ...
  await ensureStartTriggered(page);`)
    ...
    .click();
  await ensureStartTriggered(page);`);
        const normalizedModalStartClicks = normalizedStartClicks.replace(/await (\w+)\.locator\('iframe\[name\^="vfFrameId_"\]'\)\.last\(\)\.contentFrame\(\)\.getByRole\('button', \{ name: \/Start\/i \}\)\.first\(\)\.click\(\{force: true\}\);/g, `await clickModalStartIfPresent($1);
  await $1
  ...
  await ensureStartTriggered($1);`);
        const normalizedExactStartClicks = normalizedModalStartClicks.replace(/await (\w+)\.locator\('iframe\[name\^="vfFrameId_"\]'\)\.last\(\)\.contentFrame\(\)\.getByRole\('button', \{ name: 'Start' \}\)\.first\(\)\.click\(\);/g, `await clickModalStartIfPresent($1);
  await $1
    .locator('iframe[name^="vfFrameId_"]').last()
    .contentFrame()
    ...
    .click({force: true});
  await ensureStartTriggered($1);`);
        const normalizedMaintenanceWaits = normalizedExactStartClicks.replace(/await (\w+)\.locator\('iframe\[name\^="vfFrameId_"\]'\)\.last\(\)\.contentFrame\(\)\.locator\('([^']*job-start[^']*)'\)\.click\(\);\s*\n\s*await \1\.locator\('iframe\[name\^="vfFrameId_"\]'\)\.last\(\)\.contentFrame\(\)\.getByRole\('button', \{ name: 'OK' \}\)\.click\(\);/g, `await $1
    .locator('iframe[name^="vfFrameId_"]').last()
    .contentFrame()
    .locator('$2')
    .click();
  ...
  await waitForMaintenanceJob();`);
    ...
    if (await uiLinks.nth(1).count()) {
      ...
      await uiLinks.first().click({timeout: 15000, force: true});
        ...
        const normalizedQuickFind = normalizedUserInterfaceClick.replace(/await (\w+)\.getByRole\('searchbox', \{ name: 'Quick Find' \}\)\.click\(\);\s*\n\s*await \1\.getByRole\('searchbox', \{ name: 'Quick Find' \}\)\.fill\('([^']+)'\);\s*\n\s*await \1\.getByRole\('link', \{ name: '\2'(?:,\s*exact:\s*true)? \}\)(?:\.nth\(\d+\))?\.click\(\);/g, `await robustQuickFindClick($1, '$2');`).replace(/await (\w+)\.getByRole\('searchbox', \{ name: 'Quick Find' \}\)\.fill\('([^']+)'\);/g, `await $1.getByRole('searchbox', {name: 'Quick Find'}).fill('$2');\n  await $1.getByRole('searchbox', {name: 'Quick Find'}).press('Enter');`);
        const normalizedIframeHtmlClicks = normalizedQuickFind.replace(/await (\w+)\.locator\('iframe\[name\^="vfFrameId_"\]'\)\.last\(\)\.contentFrame\(\)\.locator\('html'\)\.click\(\);/g, `// omit iframe html click in patched tests to avoid timeouts in other orgs`);
        ...
        const normalizedBaseUrlExpressions = normalizedBaseUrls
            ...
            .replace(/"baseUrl(\/[^"]*)"/g, "baseUrl + '$1'");
        const normalizedCheckboxes = normalizedBaseUrlExpressions.replace(/await (\w+)\.locator\('iframe\[name\^="vfFrameId_"\]'\)\.last\(\)\.contentFrame\(\)\.getByRole\('checkbox', \{ name: '([^']+)' \}\)\.(check|uncheck)\(\);/g, `{
    ...
    if (checkbox) {
      ...
      await checkbox.$3({timeout: 15000});
        ...
        const normalizedSetupSaveClicks = normalizedCheckboxes.replace(/await (\w+)\.locator\('iframe\[name\^="vfFrameId_"\]'\)\.last\(\)\.contentFrame\(\)\.getByRole\('button', \{ name: 'Save' \}\)\.click\(\);/g, `{
    const saveButton = $1
      .locator('iframe[name^="vfFrameId_"]').last()
      .contentFrame()
    ...
    if (await saveButton.count()) {
      ...
      console.log('⚠️ Se omite Save porque no existe botón Save visible en el contexto actual.');
        ...
        const redeclarationFixed = injectedImports.replace(/const\s+(page\d+(?:Promise)?)\s*=/g, 'var $1 =');
        const baseUrlDeclaration = redeclarationFixed.includes('const baseUrl')
            ? ''
            : 'const baseUrl = process.env.METADELTA_BASE_URL;\n';
        const injectedBase = redeclarationFixed.replace(/(import\s+\{\s*test[^;]+;\n)/, `$1${baseUrlDeclaration}`);
        const popupResolutionFixed = injectedBase.replace(/(var\s+(page\d+)Promise\s*=\s*[a-zA-Z0-9_]+\.waitForEvent\('popup'\);(?:(?:\s*\/\/.*|\s*console\.log[^;]+;)*)\s*)(?=await\s+\2\.)/g, '$1var $2 = await $2Promise;\n  ');
        const setupMenuFixed = popupResolutionFixed.replace(/(await\s+([a-zA-Z0-9_]+)\.getByRole\('button',\s*\{\s*name:\s*(?:'|"|`)Setup(?:'|"|`)\s*\}\)\.click\(\);\s*\n\s*var\s+([a-zA-Z0-9_]+)Promise\s*=\s*\2\.waitForEvent\('popup'\);\s*\n)(?!\s*await\s+\2\.(?:getByRole|getByText|locator)\([^)]*(?:Setup|menuitem|new tab)[^)]*\))/gi, `$1  var setupMenuItem = $2.getByRole('menuitem', {name: /Setup Opens in a new tab/i});\n  if (await setupMenuItem.count()) {\n    await setupMenuItem.first().click();\n  }\n`);
        const injected = setupMenuFixed.replace(/(test\(['"][^'"]+['"],\s*async\s*\(\{\s*page\s*\}\)\s*=>\s*\{\s*\n)/, `$1  test.setTimeout(${Math.max(300000, (vlocityJobTime ?? 180) * 1000 + 120000)});\n  page.setDefaultTimeout(60000);\n  installOrgDomainGuard(page);\n  await page.goto(process.env.METADELTA_BASE_URL);\n  await runTaskOrchestrator(page);\n`);
        const helper = `
...
async function robustQuickFindClick(page, searchText) {
  for (let attempt = 1; attempt <= 8; attempt++) {
    ...
    const fallbackLink = page.locator('a').filter({hasText: searchText});
    ...
    try {
      ...
      if (await exactLink.count() > 0) {
        await exactLink.first().scrollIntoViewIfNeeded();
        await exactLink.first().click({timeout: 5000, force: true});
        await page.waitForTimeout(1000);
        return;
      }

      if (await fallbackLink.count() > 0) {
        await fallbackLink.first().scrollIntoViewIfNeeded();
        await fallbackLink.first().click({timeout: 5000, force: true});
        await page.waitForTimeout(1000);
        return;
      }
