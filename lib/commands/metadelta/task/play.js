...
class TaskPlay extends Command {
  ...
  await $1.getByRole('searchbox', {name: 'Quick Find'}).press('Enter');`);
        ...
        const normalizedCheckboxes = normalizedIframeHtmlClicks.replace(/await (\w+)\.locator\('iframe\[name\^="vfFrameId_"\]'\)\.contentFrame\(\)\.getByRole\('checkbox', \{ name: '([^']+)' \}\)\.(check|uncheck)\(\);/g, `{
