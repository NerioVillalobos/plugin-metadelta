  ...
  await waitForMaintenanceJob();`);
    ...
    if (await uiLinks.nth(1).count()) {
      ...
      await uiLinks.first().click({timeout: 15000, force: true});
        ...
        const normalizedQuickFindSequence = normalizedUserInterfaceClick.replace(/await (\w+)\.getByRole\('searchbox', \{ name: 'Quick Find' \}\)\.click\(\);\s*await \1\.getByRole\('searchbox', \{name: 'Quick Find'\}\)\.fill\('([^']+)'\);\s*await \1\.getByRole\('searchbox', \{name: 'Quick Find'\}\)\.press\('Enter'\);/g, `await searchInQuickFind($1, page, '$2');`);
        const normalizedQuickFind = normalizedQuickFindSequence.replace(/await (\w+)\.getByRole\('searchbox', \{ name: 'Quick Find' \}\)\.fill\('([^']+)'\);/g, `await searchInQuickFind($1, page, '$2');`);
