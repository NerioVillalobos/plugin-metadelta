  ...
  await waitForMaintenanceJob();`);
    ...
    if (await deliverabilityLink.count()) {
      ...
      await $1.getByText('Deliverability', {exact: true}).first().click({timeout: 15000});
        ...
        const normalizedQuickFindClick = normalizedDeliverabilityClick.replace(/await (\w+)\.getByRole\('searchbox', \{ name: 'Quick Find' \}\)\.click\(\);/g, `{
    const quickFindSearchbox = await ensureQuickFindSearchbox($1);
    await quickFindSearchbox.click({timeout: 20000});
  }`);
        const normalizedUserInterfaceClick = normalizedQuickFindClick.replace(/await (\w+)\.getByRole\('link', \{ name: 'User Interface' \}\)\.nth\(1\)\.click\(\);/g, `{
