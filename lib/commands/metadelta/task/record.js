...
import { TaskOrchestrator, ensureTestsDirectory, ensurePlaywrightReady, buildFrontdoorUrlFromOrgDisplay, formatTimestampForFilename, injectBaseUrlInTest, sanitizeAlias, executeNpxCommand, } from '../../../utils/task/orchestrator.js';
class TaskRecord extends Command {
    ...
    async run() {
        ...
        try {
            ...
            const result = executeNpxCommand(['--yes', 'playwright', 'codegen', url, '--target', 'playwright-test', '--output', outputPath], { stdio: 'inherit' });
            if (result.status !== 0) {
                const details = [
