...
import { TaskOrchestrator, ensureTestsDirectory, formatTimestampForFilename, injectBaseUrlInTest, sanitizeAlias, } from '../../../utils/task/orchestrator.js';
class TaskRecord extends Command {
    ...
    async run() {
        ...
        try {
            ...
            const result = spawnSync('npx', ['playwright', 'codegen', url, '--target', 'playwright-test', '--output', outputPath], { stdio: 'inherit' });
            if (result.status !== 0) {
    ...
    fetchOrgUrl(targetOrg) {
        const result = spawnSync('sf', ['org', 'open', '--url-only', '--target-org', targetOrg], { encoding: 'utf8' });
        if (result.status !== 0) {
