...
class MetadeltaSecurityUsers extends Command {
    ...
    runCompareMode(flags, matrix) {
        ...
        for (const row of validationRows) {
            ...
            const diffReasons = [];
            if (!this.setsEqual(expected.PermissionSetGroup, currentPsg))
                diffReasons.push('PermissionSetGroup');
            if (!this.setsEqual(expected.PublicGroupPuesto, currentPuesto))
                diffReasons.push('PublicGroupPuesto');
            if (!this.setsEqual(expected.PublicGroupSegmento, currentSegmento))
                diffReasons.push('PublicGroupSegmento');
            if (!this.setsEqual(expected.Queues, currentQueues))
                diffReasons.push('Queues');
