  ...
  `;
    ...
    compareFolders({ tempDir, projectRoot, vlocityDir }) {
        ...
        for (const filePath of retrievedFiles) {
            ...
            const baseContent = baseFile && fs.existsSync(baseFile)
    ...
    printTable(rows) {
        ...
        const divider = (left, middle, right) => `${left}${'─'.repeat(widths[0] + 2)}${middle}${'─'.repeat(widths[1] + 2)}${middle}${'─'.repeat(widths[2] + 2)}${right}`;
