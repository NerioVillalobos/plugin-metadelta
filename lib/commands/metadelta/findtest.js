...
const findManualStepDoc = (baseDir, identifier) => {
    ...
    while (stack.length > 0) {
        ...
        for (const entry of entries) {
            ...
            if (entry.name.toUpperCase().includes(identifier.toUpperCase()) ||
...
const isDirectTestMatch = (apexClass, testClass) => {
    ...
    if (DIRECT_TEST_SUFFIXES.some((suffix) => {
        const normalizedSuffix = suffix.replace(/[^a-z0-9]/g, '');
...
const gatherTestsForDeployment = (apexMembers, manifestMembers, mapping, classesDirectory, availableApexClasses = new Set()) => {
...
class FindTest extends Command {
    ...
    static flags = {
        ...
        'run-deploy-prod': Flags.boolean({
            summary: 'Ejecuta el despliegue sin --dry-run y evita NoTestRun cuando no hay clases de prueba (producción).'
        }),
    ...
    async run() {
        ...
        const xmlNameResolved = flags['xml-name']
            ? resolvePath(projectRoot, flags['xml-name'].endsWith('.xml') ? flags['xml-name'] : `${flags['xml-name']}.xml`)
            : null;
        ...
        if (manifestFlagPath && !manifestExists) {
            ...
            if (manualDocPath) {
                ...
                this.log(`Se detectaron pasos manuales en ${relativeManualPath}. Ejecute esos pasos manuales sin utilizar --dry-run ni --run-deploy.`);
                return;
            }
        ...
        this.log(`${summaryLabel}: ${inputClasses.length} · Filtradas: ${filteredCount} · Presentes en repo: ${finalClasses.length}`);
        if (verbose) {
            ...
            logLimitedWarnings(Array.from(ignoredManaged).sort(), (cls) => `Se omitió ${cls} por namespace gestionado (__).`);
            logLimitedWarnings(Array.from(ignoredCommunities).sort(), (cls) => `Se omitió ${cls} por pertenecer a los controladores estándar de Communities.`);
            logLimitedWarnings(missingLocal.sort(), (cls) => `Se omitió ${cls} porque no existe en el filesystem.`, 'Revise su manifest o use --only-local.');
        }
        if (finalClasses.length === 0) {
            if (usedManifest) {
                ...
                this.error('El manifest no tiene clases presentes en el repo local. Usa --only-local o pasa un manifest válido.');
            }
        }
        const metrics = {
        ...
        if (flags.deploy || manifestExists) {
            ...
            const { testsToRun, testsMissingInManifest, missingTestFiles, apexWithoutTests, missingApexClasses, lowConfidenceMatches } = gatherTestsForDeployment(finalClasses, normalizedMembers, apexTestMapping, sourceDir, availableApexClasses);
            ...
            if (testsMissingInManifest.length > 0) {
                ...
                if (apiVersionError) {
                    this.warn(`No se pudo obtener la versión de API de la org ${orgAliasForVersion ?? ''}: ${apiVersionError}`);
                }
                else {
            ...
            if (missingApexClasses.length > 0) {
                blockingWarnings.push(`No se encontraron archivos .cls para las clases Apex indicadas en el manifest: ${missingApexClasses.join(', ')}`);
            }
            ...
            if (missingTestFiles.length > 0) {
                blockingWarnings.push(`No se encontraron archivos .cls para las clases de prueba requeridas: ${missingTestFiles.join(', ')}`);
            }
            blockingWarnings.forEach((message) => this.warn(message));
            if (testsToRun.length === 0) {
                const noTestsMessage = fallbackTestLevel === null
